...
public class FriendManagementServiceImpl implements FriendManagementService {
    ...
    public Mono<AddFriend.Response> makeFriend(AddFriend.Request request) {
        Mono<UsersRelationshipEntity> rela1 =this.addRelation(request.getFriends().get(0),request.getFriends().get(1), StringConstant.FRIEND_TYPE, StringConstant.BLOCK_TYPE, MessageConstant.CAN_NOT_ADD_FRIEND);
        Mono<UsersRelationshipEntity> rela2 =this.addRelation(request.getFriends().get(1),request.getFriends().get(0), StringConstant.FRIEND_TYPE, StringConstant.BLOCK_TYPE, MessageConstant.CAN_NOT_ADD_FRIEND);
    ...
    public Mono<RetrieveFriend.Response> getListFriendByEmail(RetrieveFriend.Request request) {
        return usersRepository.findByEmail(request.getEmail())
                ...
                .flatMap(p-> usersRepository.FindByRelationTypeAndEmail(StringConstant.FRIEND_TYPE,p.getId())
                        .map(UsersEntity::getEmail).collectList()
    ...
    public Mono<RetrieveFriendBetween.Response> getFriendBetween(RetrieveFriendBetween.Request request) {
        return usersRepository.findByEmail(request.getFriends().get(0))
                .filter(Objects::nonNull)
                .flatMap(p-> usersRepository.findByEmail(request.getFriends().get(1))
                                .filter(Objects::nonNull)
                                .flatMap(f -> usersRepository.FindByRelationTypeAndTwoEmail(p.getId(), f.getId(), StringConstant.FRIEND_TYPE)
                                        .map(UsersEntity::getEmail)
                                        .collectList()
                                        .map(list -> RetrieveFriendBetween.Response.builder().success(true).friends(list).count(list.size()).build() ))
     ...
     */
    @Override
    public Mono<RetrieveFriendBetween.Response> getFriendBetween2(RetrieveFriendBetween.Request request){
        ...
        return usersRepository.findByEmail(request.getFriends().get(0))
                ...
                .flatMap(p-> usersRepository.FindByRelationTypeAndEmail(StringConstant.FRIEND_TYPE,p.getId())
                        ...
                        .collectList().zipWith(usersRepository.findByEmail(request.getFriends().get(1))
                                .filter(Objects::nonNull)
                                .flatMap(z-> usersRepository.FindByRelationTypeAndEmail(StringConstant.FRIEND_TYPE,p.getId())
                                        .map(UsersEntity::getEmail)
                                        .collectList())
                                .switchIfEmpty(monoResponseStatusNotFoundException(request.getFriends().get(1))))
                        .map(tuple2 ->{
     ...
     */
    private  Mono<UsersRelationshipEntity> addRelation (String email1, String email2, Integer type1, Integer type2, String message) {
            ...
            return usersRepository.findByEmail(email1)
                    ...
                    .flatMap(p-> usersRepository.findByEmail(email2)
                            ...
                            .flatMap(z -> usersRelationshipRepository.checkRelation(p.getId(), z.getId(), type1, type2)
                                    ...
                                    .flatMap(c-> Mono.just(UsersRelationshipEntity.builder()
                                            .userId(p.getId())
    ...
    public Mono<RetrieveReceiveUpdate.Response> retrieveEmailReceiveUpdate(RetrieveReceiveUpdate.Request request){
        return usersRepository.getAllUserBlockAnEmail(StringConstant.BLOCK_TYPE,request.getSender())
                .map(UsersEntity::getEmail).collectList()
